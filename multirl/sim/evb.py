...
def evb_run(pdb, ref_pdb, lig_yml, template_yml,
            pymol_exec='pymol', wham_exe='wham_exe', amber_bin=''):
...
def evb_analysis(
        md_paths: list,
        skip_start: int = 100,
        wham_exe: str = None) -> np.ndarray:
    """Perform the EVB analysis

    Args:
        md_paths: Paths to the MD simulation directories
        skip_start: How many measurements to exclude from the beginning of each MD run
        wham_exe: Path to the WHAM executable. Set to `None` to use the

    Returns:
        PMF as a numpy array
    """
    if wham_exe is None:
        wham_exe = config.WHAM_PATH
